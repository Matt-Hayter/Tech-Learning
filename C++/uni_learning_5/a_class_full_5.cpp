...
public:
  ...
  //Not sure why const and set is used here as we are within class. x y and z could just be used?
  vector3 plus(const vector3 &v) const // const used so set_x is needed to change values
  {
    vector3 temp;
    temp.set_x(x+v.x);
    temp.set_y(y+v.y);
