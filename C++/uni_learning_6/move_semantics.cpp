...
public:
  ...
  size_t length() const {return size;} //get() equivalent
...
// Move constructor
//Need to access rvalue address (rigth hand of =), so need &&.
//Wanted to change it's values, so need address
dynamic_array::dynamic_array(dynamic_array &&arr)
{ // steal the data
  std::cout <<"move constructor\n";
  size=arr.size;
  //Don't have to run through each element as we have arr's address.
  array=arr.array;
  arr.size=0;
  arr.array=nullptr;
}
...
dynamic_array & dynamic_array::operator=(dynamic_array&& arr)
{
  ...
  return *this; // Return object in question
}
