...
//Highest level of protection used by default (from either bas or derived class)
class star: public celestial_object //Public so public celestisl_object data is not made more protected(could cause problems in later derived classes)
{
protected:
...
public:
  ...
  neutron_star(const std::string nm, const double l, const double m, const double d,
           const double r) :
    star{nm, l, m, d, "pulsar"}, radius{r} {} //Paramaterised constructor uses all inputs of star object plus an input for it's memeber data
  ~neutron_star(){}
...
std::ostream& operator<<(std::ostream& o, const neutron_star& st)
{
  ...
  o << static_cast<star>(st); //Accesses the inherited class
  return o;
}
...
int main()
{
  ...
  //prints at every inherited level due to static_cast<> used in each function.
  std::cout << crab;
  return 0;
}
